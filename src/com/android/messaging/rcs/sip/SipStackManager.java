...
public class SipStackManager {
    ...
    public void connectAndRegister() {
        new Thread(() -> {
            try {
                LogUtil.i(TAG, "Opening TLS Socket to P-CSCF: " + mConfig.getPCscfAddress() + ":" + mConfig.getPCscfPort());
                final SSLSocketFactory factory = (SSLSocketFactory) SSLSocketFactory.getDefault();
                mSocket = factory.createSocket(mConfig.getPCscfAddress(), mConfig.getPCscfPort());
    ...
    public void sendRegister(String nonce, String realm) {
        try {
            ...
            sb.append("Via: SIP/2.0/TLS ").append(mSocket.getLocalAddress().getHostAddress()).append(":")
                    .append(mSocket.getLocalPort()).append(";branch=").append(branch).append("\r\n");
            ...
            sb.append("Contact: <sip:").append(mConfig.getDigestUsername()).append("@")
                    .append(mSocket.getLocalAddress().getHostAddress()).append(">;+g.3gpp.iari-ref=\"urn%3Aurn-7%3A3gpp-application.ims.iari.rcse.dp\"\r\n");
    ...
    private void startIncomingReaderThread() {
        new Thread(() -> {
            ...
            try {
                int bytesRead;
                while (mIsConnected.get() && (bytesRead = mInputStream.read(buffer)) != -1) {
